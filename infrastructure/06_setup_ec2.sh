...
aws s3 cp load-generation/load_generator.py "${S3_STAGING}/load_generator.py" --region "$REGION"
aws s3 cp load-generation/validate_endpoints.py "${S3_STAGING}/validate_endpoints.py" --region "$REGION"
aws s3 cp load-generation/smoke_test.py "${S3_STAGING}/smoke_test.py" --region "$REGION" 2>/dev/null || true
aws s3 cp load-generation/generate_payloads.py "${S3_STAGING}/generate_payloads.py" --region "$REGION" 2>/dev/null || true
